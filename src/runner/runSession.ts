...
import { startOpenAIProxy, patchCodexConfig } from '../proxy/openaiProxy';
import { getSessionResponse, updateSessionStatus, saveAgentSessionId, appendSessionOutput } from '../api/backendClient';
...
function stripPtyForStorage(raw: string): string {
  const text = raw
...
async function saveChunked(sessionId: string, text: string, stream: 'stdout' | 'stderr'): Promise<void> {
  for (let i = 0; i < text.length; i += PTY_CHUNK_MAX) {
    await appendSessionOutput(sessionId, text.slice(i, i + PTY_CHUNK_MAX), stream).catch(() => {});
  }
}
...
export async function runSession(options: RunSessionOptions): Promise<void> {
  ...
  if (preKnownAgentSessionId) {
    void saveAgentSessionId(state.id, preKnownAgentSessionId).catch(() => {});
  }
  ...
  const codexRestoreConfig = openaiProxy
    ? await patchCodexConfig(openaiProxy.port).catch(() => null)
    : null;
  ...
  const proxyActive =
    (anthropicProxy !== null && adapter.usesAnthropicProxy) ||
  ...
  let responsePoller: ReturnType<typeof setInterval> | null = null;
  startResponsePolling();

  function startResponsePolling(): void {
    if (responsePoller) return;
    responsePoller = setInterval(async () => {
      try {
        const response = await getSessionResponse(state.id);
        if (!response) return;
  ...
  function stopResponsePolling(): void {
    if (responsePoller) {
  ...
  async function writeToPty(response: string): Promise<void> {
    ...
    if (!proxyActive) {
      ...
      appendSessionOutput(state.id, response, 'stderr')
  ...
  term.onData((data: string) => {
    ...
    if (!proxyActive) ptyOutputAccum += data;
    ...
    if (!agentSessionIdSaved && startupBuf.length < STARTUP_SCAN_LIMIT) {
      ...
      if (detected) {
        ...
        void saveAgentSessionId(state.id, detected).catch(() => {});
    ...
    if (next !== null) {
      ...
      if (next === 'waiting_for_input' || next === 'approval_required') {
        ...
        if (!proxyActive && hasUserSentMessage && ptyOutputAccum) {
          ...
          if (cleaned) {
            saveChunked(state.id, cleaned, 'stdout')
              ...
              .catch((e: unknown) => dbg(`saveChunked FAILED: ${e}`));
          } else {
        ...
        setTimeout(() => {
          ...
          void updateSessionStatus(state.id, 'waiting_for_input', {
            question,
  ...
  function isRealUserInput(chunk: Buffer): boolean {
    for (const byte of chunk) {
      if (byte >= 0x20 && byte <= 0x7e) return true; // printable ASCII
      if (byte === 0x0d || byte === 0x0a) return true; // Enter
      if (byte === 0x7f || byte === 0x08) return true; // Backspace/DEL
      if (byte === 0x03 || byte === 0x04) return true; // Ctrl+C / Ctrl+D
    }
    return false;
  }
  ...
  return new Promise((resolve) => {
    term.onExit(async ({ exitCode, signal }) => {
      ...
      if (codexRestoreConfig) await codexRestoreConfig();
